 ...
 *  and need VC2015 or a C++11 compiler. With a few correction can be compiler
 *by VC2013
 */

// http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2014/n4100.pdf      ---
// pdf of std draft N4100 <filesystem> 2014-07-04
// http://en.cppreference.com/w/cpp/experimental/fs
// http://cpprocks.com/introduction-to-tr2-filesystem-library-in-vs2012/  ---
// TR2 filesystem in VS2012
// https://msdn.microsoft.com/en-us/library/hh874694%28v=vs.140%29.aspx   ---
// C++ 14, the <filesystem> header VS2015
// https://msdn.microsoft.com/en-us/library/hh874694%28v=vs.120%29.aspx   ---
// <filesystem> header VS2013
// http://cplusplus.github.io/filesystem-ts/working-draft.html            --- in
// html format
// http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2014/n4099.html     --- in
// html format http://article.gmane.org/gmane.comp.lib.boost.devel/256220 ---
// The filesystem TS unanimously approved by ISO.
// http://theboostcpplibraries.com/boost.filesystem                       ---
// Boost docs http://www.boost.org/doc/libs/1_58_0/libs/filesystem/doc/index.htm
...
// --- Table 1.4. g++ C++ Technical Specifications Implementation Status
...
//#define NANA_FILESYSTEM_FORCE 1
     ...
     ((defined(STD_FILESYSTEM_NOT_SUPPORTED) &&                                \
       !defined(BOOST_FILESYSTEM_AVAILABLE)) &&                                \
      !(defined(BOOST_FILESYSTEM_FORCE) || defined(STD_FILESYSTEM_FORCE))))
#undef NANA_USING_NANA_FILESYSTEM
#define NANA_USING_NANA_FILESYSTEM 1

#elif (defined(BOOST_FILESYSTEM_AVAILABLE) &&                                  \
       (defined(BOOST_FILESYSTEM_FORCE) ||                                     \
        (defined(STD_FILESYSTEM_NOT_SUPPORTED) &&                              \
...
//
// Boost versions: 1.67.0, 1.66.0, ... 1.56.0 enable directory_iterator C++11
// range-base for
...
#if ((defined(_MSC_VER) && (_MSC_VER >= 1912) && defined(_MSVC_LANG) &&        \
      _MSVC_LANG >= 201703)) ||                                                \
    ((__cplusplus >= 201703L) &&                                               \
...
enum class file_type {
  none = 0, ///< has not been determined or an error occurred while trying to
            ///< determine
  not_found =
      -1, ///< Pseudo-type: file was not found. Is not considered an error
  regular = 1,
  directory = 2,
  ...
  socket = 7,
  unknown = 8 ///< The file does exist, but is of an operating system dependent
              ///< type not covered by any of the other
};
...
enum class perms {
  ...
  unknown = 0xFFFF ///<  not known, such as when a file_status object is created
                   ///<  without specifying the permissions
};
...
using file_time_type =
    std::chrono::time_point<std::chrono::system_clock>; ///< trivial-clock> ;
...
/// concerned only with lexical and syntactic aspects and does not necessarily
/// exist in external storage, and the pathname is not necessarily valid for the
/// current operating system or for a particular file system A sequence of
/// elements that identify the location of a file within a filesystem. The
/// elements are the: rootname (opt), root-directory (opt), and an optional
/// sequence of filenames. The maximum number of elements in the sequence is
...
public:
  ...
  template <typename Source> path(const Source &source) { _m_assign(source); }
...
public:
  ...
  filesystem_error(const std::string &msg, const path &path1,
                   std::error_code err);

  filesystem_error(const std::string &msg, const path &path1, const path &path2,
                   std::error_code err);
...
/// InputIterator that iterate over the sequence of directory_entry elements
/// representing the files in a directory, not an recursive_directory_iterator
...
private:
  template <typename Char> static bool _m_ignore(const Char *p) {
    while (*p == '.')
      ++p;
    return (*p == 0);
  }
...
// noexcept; recursive_directory_iterator end(const
// recursive_directory_iterator&) noexcept;

// template<typename Value_Type>
inline bool operator==(const directory_iterator /*<Value_Type>*/ &x,
                       const directory_iterator /*<Value_Type>*/ &y) {
  return x.equal(y);
}
...
// bool create_directory(const path& p, const path& attributes,     error_code&
// ec) noexcept;

/// The time of last data modification of p, determined as if by the value of
/// the POSIX stat structure member st_mtime obtained as if by POSIX stat().
...
parent_path(const std::basic_string<CharType> &path) {
...
#else // not #if NANA_USING_NANA_FILESYSTEM and not BOOST_FILESYSTEM, also
      // incomplete STD_FILESYSTEM
// Implements the missing functions for various version of
...
*/
// Visual Studio 2017
#if (defined(NANA_USING_STD_EXPERIMENTAL_FILESYSTEM) && defined(_MSC_VER) &&   \
     (_MSC_VER > 1912)) ||                                                     \
    (!defined(__clang__) && defined(__GNUC__) &&                               \
     (__cplusplus < 201603 || (__GNUC__ * 100 + __GNUC_MINOR__ < 801)))
path weakly_canonical(const path &p);
path weakly_canonical(const path &p, std::error_code &err);
#endif
} // namespace filesystem
