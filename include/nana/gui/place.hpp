 ...
 *
 *	@contributions
 *	error, width/height, min/max and splitter bar initial weight by Ariel
 *Vina-Rodriguez.
 */
...
template <typename Widget> class agent : public detail::place_agent {
public:
  agent(std::function<void(Widget &)> initializer)
      : init_(std::move(initializer)) {}

  agent(const char *text) : text_(text) { throw_not_utf8(text); }
...
///  Layout management - an object of class place is attached to a widget, and
///  it automatically positions and resizes the children widgets.
...
public:
  ...
  public:
    error(const std::string &what, const place &plc,
          std::string field = "unknown",
          std::string::size_type pos = std::string::npos);
    std::string base_what;
    std::string
        owner_caption;    ///< truncate caption (title) of the "placed" widget
    std::string div_text; ///< involved div_text
    std::string field;    ///< posible field where the error ocurred.
    std::string::size_type pos; ///< posible position in the div_text where the
                                ///< error ocurred. npos if unknown
  };
  ///  reference to a field manipulator which refers to a field object created
  ///  by place
   ...
   *	@remark	It will throw an exception if the place has already binded to a
   *window.
   */
  void bind(window handle);
  window window_handle() const;

  void splitter_renderer(
      std::function<void(window, paint::graphics &, mouse_action)> fn);

  void div(std::string div_text); ///< Divides the attached widget into fields.
                                  ///< May throw placa::error
  const std::string &
  div() const noexcept; ///< Returns div-text that depends on fields status.
  static bool valid_field_name(const char *name) ///< must begin with _a-zA-Z
  {
    return name && (*name == '_' || (('a' <= *name && *name <= 'z') ||
                                     ('A' <= *name && *name <= 'Z')));
  }
  void modify(const char *field_name,
              const char *div_text); ///< Modifies a specified field. May throw
                                     ///< placa::error

  field_reference
  field(const char *name); ///< Returns a field with the specified name.

  void field_visible(const char *field_name,
                     bool visible); ///<<Shows/Hides an existing field.
  bool field_visible(const char *field_name)
      const; ///< Determines whether the specified field is visible.

  void field_display(const char *field_name,
                     bool display); ///< Displays/Discards an existing field.
  bool field_display(const char *field_name)
      const; ///< Determines whether the specified field is displayed.
  ...
  field_reference
  operator[](const char *name); ///< Returns a field with the specified name.
                                ///< Equal to field();

  /// Add a panel factory
  template <typename Panel, typename... Args>
  place &dock(const std::string &dockname, const std::string &factory_name,
              Args &&...args) {
    return dock(dockname, factory_name,
                std::bind(
                    [](window parent, Args &...args) {
                      return std::unique_ptr<widget>(
                          new Panel(parent, std::forward<Args>(args)...));
                    },
                    std::placeholders::_1, args...));
  }

  place &dock(const std::string &dockname, std::string factory_name,
