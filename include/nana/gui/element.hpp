...
class element_interface : public detail::element_abstract {
public:
  virtual bool draw(graph_reference, const nana::color &bgcolor,
                    const nana::color &fgcolor, const nana::rectangle &,
                    element_state) = 0;
};

class crook_interface : public detail::element_abstract {
public:
  ...
  virtual bool draw(graph_reference, const nana::color &bgcolor,
                    const nana::color &fgcolor, const nana::rectangle &,
                    element_state, const data &) = 0;
};

class border_interface : public detail::element_abstract {
public:
  virtual bool draw(graph_reference, const ::nana::color &bgcolor,
                    const ::nana::color &fgcolor, const ::nana::rectangle &,
                    element_state, unsigned weight) = 0;
};

class arrow_interface : public detail::element_abstract {
public:
  virtual bool draw(graph_reference, const ::nana::color &bgcolor,
                    const ::nana::color &fgcolor, const ::nana::rectangle &,
                    element_state, direction) = 0;
};

class provider {
public:
  template <typename ElementInterface>
  struct factory_interface : public detail::factory_abstract {
    virtual ElementInterface *create() const = 0;
  };

  template <typename Element, typename ElementInterface>
  class factory : public factory_interface<ElementInterface> {
  public:
    using interface_type = factory_interface<ElementInterface>;

    ElementInterface *create() const override { return (new Element); }
  };

  void add_arrow(const std::string &,
                 const pat::cloneable<factory_interface<arrow_interface>> &);
  arrow_interface *const *cite_arrow(const std::string &);

  void add_border(const std::string &,
                  const pat::cloneable<factory_interface<border_interface>> &);
  border_interface *const *cite_border(const std::string &);

  void add_button(const std::string &,
                  const pat::cloneable<factory_interface<element_interface>> &);
  ...
  void add_crook(const std::string &name,
                 const pat::cloneable<factory_interface<crook_interface>> &);
...
class arrow;
template <typename ArrowElement> void add_arrow(const std::string &name) {
  using factory_t = provider::factory<ArrowElement, arrow_interface>;
  provider().add_arrow(
      name, pat::cloneable<typename factory_t::interface_type>(factory_t()));
}

class border;
template <typename BorderElement> void add_border(const std::string &name) {
...
class button;
template <typename ButtonElement> void add_button(const std::string &name) {
...
class x_icon;
template <typename UserElement> void add_x_icon(const std::string &name) {
...
class crook;
template <typename UserElement> void add_crook(const std::string &name) {
...
class cross;
template <typename UserElement> void add_cross(const std::string &name) {
  using factory_t = provider::factory<UserElement, element_interface>;
  provider().add_cross(
      name, pat::cloneable<typename factory_t::interface_type>(factory_t()));
}
...
template <> class facade<element::crook> : public element::element_interface {
public:
  using graph_reference = ::nana::paint::graphics &;
...
public:
  ...
  bool draw(graph_reference, const nana::color &bgcolor,
            const nana::color &fgcolor, const nana::rectangle &r,
            element_state) override;
...
template <> class facade<element::cross> : public element::element_interface {
public:
  facade(const char *name = nullptr);
  void switch_to(const char *);
...
template <> class facade<element::border> : public element::element_interface {
  using graph_reference = ::nana::paint::graphics &;
...
template <> class facade<element::arrow> : public element::element_interface {
  using graph_reference = ::nana::paint::graphics &;

public:
  enum class style { solid };
...
template <> class facade<element::button> : public element::element_interface {
public:
  facade(const char *name = nullptr);
  void switch_to(const char *);
...
template <> class facade<element::x_icon> : public element::element_interface {
public:
  facade(const char *name = nullptr);
  void switch_to(const char *);
...
public:
  ...
  bool draw(graph_reference, const nana::color &bgcolor,
            const nana::color &fgcolor, const nana::rectangle &, element_state);
...
class bground : public element_interface {
public:
  typedef paint::graphics &graph_reference;
  ...
  bground &image(
      const paint::image &, bool vertical,
      const nana::rectangle &valid_area); ///< Set a picture for the background
  bground &image(
      const paint::graphics &, bool vertical,
      const nana::rectangle &valid_area); ///< Set a picture for the background
  ...
  void stretch_parts(unsigned left, unsigned top, unsigned right,
                     unsigned bottom);
