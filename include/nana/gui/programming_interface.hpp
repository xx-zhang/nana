...
make_scheme(::nana::detail::scheme_factory_interface &&);
}

void effects_edge_nimbus(window, effects::edge_nimbus);
effects::edge_nimbus effects_edge_nimbus(window);

void effects_bground(window, const effects::bground_factory_interface &,
                     double fade_rate);
void effects_bground(std::initializer_list<window> wdgs,
                     const effects::bground_factory_interface &,
                     double fade_rate);
...
// namespace dev
//@brief: The interfaces defined in namespace dev are used for developing the
// nana.gui
namespace dev {
...
template <typename Scheme> std::unique_ptr<Scheme> make_scheme() {
  return std::unique_ptr<Scheme>{static_cast<Scheme *>(
      API::detail::make_scheme(::nana::detail::scheme_factory<Scheme>()))};
}
...
bool copy_transparent_background(window, const rectangle &src_r,
                                 paint::graphics &, const point &dst_pt);

/// Refreshes a widget surface
/*
 * This function will copy the drawer surface into system window after the event
 * process finished.
 */
void lazy_refresh();

void draw_shortkey_underline(paint::graphics &, const std::string &text,
                             wchar_t shortkey, std::size_t shortkey_position,
                             const point &text_pos, const color &);
...
class enum_widgets_function : public enum_widgets_function_base {
public:
...
private:
  ...
  template <typename T, typename std::enable_if<std::is_same<
                            ::nana::widget, T>::value>::type * = nullptr>
...
/// Sets languages
/**
 * Specifies the languages in order to make the program display multi-languages
 * correctly Under Windows, the pragram can display multi-languages correctly,
 * so this function is useless for Windows.
 */
...
/// @brief	Searches whether the text contains a '&' and removes the
/// character for transforming.
///			If the text contains more than one '&' characters, the
/// others are ignored. e.g 			text = "&&a&bcd&ef", the result
/// should be "&abcdef", shortkey = 'b', and pos = 2.
std::string transform_shortkey_text(
    ...
    std::string::size_type
        *skpos ///< retrieves the shortkey position if it is not a null_ptr;
);
bool register_shortkey(window, unsigned long);
void unregister_shortkey(window);

nana::point cursor_position();
rectangle make_center(unsigned width,
                      unsigned height); ///< Retrieves a rectangle which is in
                                        ///< the center of the screen.
rectangle make_center(window, unsigned width,
                      unsigned height); ///< Retrieves a rectangle which is in
...
void enum_widgets(window wd, bool recursive, EnumFunction &&fn) {
  static_assert(
      std::is_convertible<typename std::decay<Widget>::type *,
                          ::nana::widget *>::value,
      "enum_widgets<Widget>: The specified Widget is not a widget type.");

  detail::enum_widgets_function<Widget, EnumFunction> enum_fn(
      static_cast<EnumFunction &&>(fn));
  enum_fn.enum_widgets(wd, recursive);
}

void window_icon_default(const paint::image &small_icon,
                         const paint::image &big_icon = {});
void window_icon(window, const paint::image &small_icon,
                 const paint::image &big_icon = {});

bool empty_window(window); ///< Determines whether a window is existing.
bool is_window(window); ///< Determines whether a window is existing, equal to
                        ///< !empty_window.
...
/// conversion can be employed between nana::native_window_type and HWND through
/// reinterpret_cast operator. Under X System, a conversion can be employed
/// between nana::native_window_type and Window through reinterpret_cast
/// operator. \return If the function succeeds, the return value is the native
/// window handle to the Nana.GUI window. If fails return zero.
native_window_type root(window);
window root(
    native_window_type); ///< Retrieves the native window of a Nana.GUI window.
...
typename ::nana::dev::widget_traits<Widget>::event_type &events(window wd) {
  ...
  if (nullptr == general_evt)
    throw std::invalid_argument("API::events(): bad parameter window handle, "
                                "no events object or invalid window handle.");
  ...
  if constexpr (std::is_same_v<event_type, ::nana::general_events>) {
    ...
    if (nullptr == widget_evt)
      throw std::invalid_argument(
          "API::events(): bad template parameter Widget, the widget type and "
          "window handle do not match.");
    return *widget_evt;
  }
#else
...
template <typename EventArg,
          typename std::enable_if<std::is_base_of<
              ::nana::event_arg, EventArg>::value>::type * = nullptr>
...
typename ::nana::dev::widget_traits<Widget>::scheme_type &scheme(window wd) {
  ...
  if (nullptr == wdg_colors)
    throw std::invalid_argument("API::scheme(): bad parameter window handle, "
                                "no events object or invalid window handle.");
  ...
  if constexpr (std::is_same<::nana::widget_geometrics, scheme_type>::value) {
    ...
    if (nullptr == comp_wdg_colors)
      throw std::invalid_argument(
          "API::scheme(): bad template parameter Widget, the widget type and "
          "window handle do not match.");
...
bool set_window_z_order(window wd, window wd_after,
...
bool track_window_size(window, const size &,
                       bool true_for_max); ///< Sets the minimum or maximum
                                           ///< tracking size of a window.
void window_enabled(window, bool);
bool window_enabled(window);

/// Refresh the window and display it immediately calling the refresh function
/// of its drawer_trigger.
/*
 * The drawer::refresh() will be called. If the current state is lazy_refrsh,
 * the window is delayed to update the graphics until an event is finished.
 * @param window_handle A handle to the window to be refreshed.
 */
void refresh_window(window window_handle);
void refresh_window_tree(
    window); ///< Refreshes the specified window and all its children windows,
             ///< then displays it immediately
void update_window(window); ///< Copies the off-screen buffer to the screen for
                            ///< immediate display.
...
/// Determines whether the specified window will get the keyboard focus when its
/// root window gets native system focus.
 ...
 * @param ignore_children Indicates whether to redirect the mouse input to its
 * children if the mouse pointer is over its children.
 */
...
/// Blocks the execution and other windows' messages until the specified window
/// is closed.
...
/**
 * This function returns an object to operate caret. The object doesn't create
 * or destroy the caret. When you are finished with the caret, be sure to reset
 * the pointer.
 *
 * @param window_handle A handle to a window whose caret is to be retrieved
 * @return a pointer to the caret proxy.
 * @except throws std::runtime if the window doesn't have a caret when
 * disable_throw is false
 */
::std::unique_ptr<caret_interface> open_caret(window window_handle,
                                              bool disable_throw = false);

/// Enables that the user can give input focus to the specified window using TAB
/// key.
void tabstop(window);

/// Enables or disables a window to receive a key_char event for pressing TAB
/// key.
/*
 * @param window_handle A handle to the window to catch TAB key through key_char
 * event.
 * @param enable Indicates whether to enable or disable catch of TAB key. If
 * this parameter is *true*, the window receives a key_char event when pressing
 * TAB key, and the input focus is not changed. If this parameter is *false*,
 * the input focus is changed to the next tabstop window.
 */
void eat_tabstop(window window_handle, bool enable);

/// Sets the input focus to the window which the tabstop is near to the
/// specified window.
/*
 * @param window_handle A handle to the window.
 * @param forward Indicates whether forward or backward window to be given the
 * input focus.
 * @return A handle to the window which to be given the input focus.
 */
window move_tabstop(window window_handle, bool forward);

/// Sets the window active state. If a window active state is false, the window
/// will not obtain the focus when a mouse clicks on it which will be obtained
/// by take_if_has_active_false.
...
bool calc_screen_point(
    window, point &); ///< Converts window coordinates to screen coordinates
bool calc_window_point(
    window, point &); ///< Converts screen coordinates to window coordinates.

window find_window(const nana::point &mspos);

bool is_window_zoomed(
    window,
    bool ask_for_max); ///< Tests a window whether it is maximized or minimized.

void widget_borderless(window,
                       bool);   ///< Enables or disables a borderless widget.
...
bool ignore_mouse_focus(window,
                        bool ignore); ///< Enables/disables the mouse focus, it
                                      ///< returns the previous state
bool ignore_mouse_focus(
    window); ///< Determines whether the mouse focus is enabled
 ...
 * @param limited_px Specifies the max pixels of width or height. If this
 * parameter is zero, this parameter will be ignored.
 * @param limit_width Indicates whether the it limits the width or height. If
 * this parameter is *true*, the width is limited. If the parameter is *false*,
 * the height is limited. This parameter is ignored if limited_px = 0.
 * @return if optional has a value, the first size indicates the content extent,
 * the second size indicates the size of widget by the content extent.
 */
::std::optional<std::pair<::nana::size, ::nana::size>>
