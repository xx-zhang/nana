...
/// Prefabricated modal dialog box (with text, icon and actions buttons) that
...
public:
  ...
  /// Default constructor that creates a message box with default title and
  /// default button, the default button is OK.
  msgbox();
  ...
  /// Constructor that creates a message box with a specified title and default
  /// button.
  msgbox(const ::std::string &);

  /// Constructor that creates a message box with an owner window, a specified
  /// title and buttons.
  ...
  template <typename T> msgbox &operator<<(const T &t) {
    sstream_ << t;
    return *this;
  }
  ...
  pick_t operator()() const { return show(); }
...
///
/// The input value can be a boolean, string, a number, an option from a
/// dropdown list or a date.
class inputbox {
...
public:
  ...
  class boolean : public abstract_content {
    struct implement;
  ...
  /// Integer input
  class integer : public abstract_content {
    struct implement;
  ...
  class real : public abstract_content {
    struct implement;

  public:
    real(::std::string label, double init_value, double begin, double last,
         double step);
    ~real();
  ...
  class text : public abstract_content {
    struct implement;
  ...
  /// Date input
  class date : public abstract_content {
    struct implement;
  ...
  /// button, it invokes the filebox with proper configurations to query a
  /// filename.
  class path : public abstract_content {
    struct implement;
  ...
  inputbox(
      window owner, ///< A handle to an owner window (just a parent form or
                    ///< widget works)
      ::std::string description, ///< tells users what the purpose for the
                                 ///< input. It can be a formatted-text.
  ...
  void image(::nana::paint::image image, ///< The image
             bool is_left, ///< true to place the image at left side, false to
                           ///< the right side
  ...
  void image_v(::nana::paint::image, ///< The image
               bool is_top, ///< `true` to place the image at top side, `false`
                            ///< at bottom side
               const rectangle &valid_area = {}
               ///< The area of the image to be displayed
  );
  ...
  ///
  /// This method shows the inputbox without preventing the user interacts with
  /// other windows.
  template <typename... Args> bool show(Args &&...args) {
  ...
  template <typename... Args> bool show_modal(Args &&...args) {
    std::vector<abstract_content *> contents;
#ifdef __cpp_fold_expressions
    (contents.emplace_back(&args), ...);
#else
...
private:
  ...
  void _m_fetch_args(std::vector<abstract_content *> &contents,
                     abstract_content &content, Args &&...args) {
