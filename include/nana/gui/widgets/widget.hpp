...
public:
  ...
  virtual window handle() const = 0; ///< Returns the handle of window, returns
                                     ///< 0 if window is not created.
  bool
  empty() const; ///< Determines whether the manipulator is handling a window.
  void close();
  ...
  template <typename... Args> void i18n(std::string msgid, Args &&...args) {
    _m_caption(::nana::to_nstring(::nana::internationalization().get(
  ...
  bool enabled() const; ///< Determines whether the window is enabled for mouse
                        ///< and keyboard input.
  void enabled(bool);

  void enable_dropfiles(
      bool); ///< Enables/Disables a window to accept dropped files.
  ...
  void umake_event(
      event_handle eh) const; ///< Deletes an event callback by a handle.

  widget &register_shortkey(
      wchar_t); ///< Registers a shortkey. To remove a registered key, pass 0.
...
protected:
  // protected members, a derived class must call this implementation if it
  // overrides an implementation
...
/// Base class of all the classes defined as a widget window. Defaultly a
/// widget_tag
///
/// \tparam Category
...
template <typename Category, typename DrawerTrigger,
          ...
          typename = typename std::enable_if<std::is_base_of<
              ::nana::drawer_trigger, DrawerTrigger>::value>::type>
...
public:
  ...
  widget_object()
    ...
    static_assert(
        std::is_base_of<::nana::drawer_trigger, DrawerTrigger>::value,
        "The type DrawerTrigger must be derived from nana::drawer_trigger");
  }

  ~widget_object() { API::close_window(handle()); }

  event_type &events() const { return *events_; }

  bool create(window parent_wd,
              bool visible) ///< Creates a no-size (zero-size) widget. in a
                            ///< widget/root window specified by parent_wd.
  {
    return create(parent_wd, rectangle(), visible);
  }

  bool create(
      window parent_wd, const rectangle &r = {},
      bool visible = true) ///< in a widget/root window specified by parent_wd.
  {
  ...
  bool borderless() const { return API::widget_borderless(handle_); }
  ...
  void filter_event(const std::vector<event_code> evt_codes,
  ...
  void clear_filter() { trigger_.clear_filter(); }
  ...
  event_filter_status filter_event() { return trigger_.filter_event(); }
...
private:
  general_events &_m_get_general_events() const override { return *events_; }
...
/// Base class of all the classes defined as a non-graphics-buffer widget
/// window.
///
/// The second template parameter DrawerTrigger is always ignored.\see
/// nana::panel type DrawerTrigger must be derived from nana::drawer_trigger
...
public:
  ...
  bool create(
      window parent_wd, const rectangle &r = rectangle(),
      bool visible = true) ///< in a widget/root window specified by parent_wd.
  {
    if (parent_wd && this->empty()) {
...
public:
  ...
  widget_object()
      : widget_object(nullptr, false, API::make_center(300, 150), appearance(),
                      this) {
    static_assert(
        std::is_base_of<::nana::drawer_trigger, DrawerTrigger>::value,
        "The type DrawerTrigger must be derived from nana::drawer_trigger");
  ...
  widget_object(window owner, bool nested, const rectangle &r = {},
  ...
  void activate() { API::activate_window(handle_); }

  native_window_type native_handle() const { return API::root(handle_); }

  void bring_top(bool activated) { API::bring_top(handle(), activated); }

  window owner() const { return API::get_owner_window(handle_); }

  void icon(const nana::paint::image &ico) { API::window_icon(handle_, ico); }

  void restore() { API::restore_window(handle_); }

  void zoom(bool ask_for_max) { API::zoom_window(handle_, ask_for_max); }

  bool is_zoomed(bool check_maximized) const {
    return API::is_window_zoomed(handle_, check_maximized);
  }

  widget_object &z_order(window wd_after,
