...
class button
  ...
  typedef widget_object<category::widget_tag, drawerbase::button::trigger>
      base_type;
...
public:
  ...
  button(window parent, const nana::rectangle & = rectangle(),
         bool visible = true);

  /// Shows an icon in front of caption
  /**
   * @param image Icon to be shown. If image is empty, the current icon is
   * erased from the button.
  ...
  bool pushed() const;
  button &pushed(bool);
  button &omitted(bool); ///< Enables/Disables omitting displaying the caption
                         ///< if the text is too long.
  button &enable_focus_color(
      bool); ///< Enables/Disables showing the caption with a special color to
             ///< indicate the button is focused.

  button &set_bground(const pat::cloneable<element::element_interface>
                          &); ///< Sets a user-defined background element.
  button &set_bground(const std::string &); ///< Sets a pre-defined background
                                            ///< element by a name.
