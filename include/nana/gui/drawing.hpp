...
/// If a picture need to be displayed immediately call
...
public:
  ...
  using draw_fn_t =
      std::function<void(paint::graphics &)>; ///< A function to draw
  ...
  /// Draws things that are defined by draw_fn_t but will not be deleted when
  /// clear() is called.
