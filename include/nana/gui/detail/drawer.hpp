...
//			a widget.
class drawer : nana::noncopyable, nana::nonmovable {
  enum class method_state { pending, overridden, not_overridden };
...
public:
  ...
  void
  map(window, bool forced,
      const rectangle *update_area = nullptr); // Copy the root buffer to screen
  void refresh();
  ...
  void _m_emit(event_code evt_code, const Arg &arg, Mfptr mfptr,
    ...
    if (realizer && (method_state::not_overridden != mth_state)) {
      const bool bFiltered =
          !bForce__EmitInternal && realizer->filter_event(evt_code);
      if (method_state::pending == mth_state) {
        if (!bFiltered)
          (realizer->*mfptr)(graphics, arg);

        // Check realizer, when the window is closed in that event handler, the
        // drawer will be detached and realizer will be a nullptr
        if (realizer)
          mth_state = (realizer->_m_overridden(evt_code)
                           ? method_state::overridden
                           : method_state::not_overridden);
      } else {
        if (!bFiltered)
          (realizer->*mfptr)(graphics, arg);
      }
