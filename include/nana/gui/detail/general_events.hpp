...
struct docker_base : public event_docker_interface {
...
class event_base : public detail::event_interface {
public:
  ~event_base();
...
protected:
  ...
  // It is used for avoiding a try{}catch block which is required for some
  // finial works when event handlers throw exceptions. Precondition
  // event_base.dockers_ != nullptr.
  class emit_counter {
  public:
...
public:
  ...
  /// ignorable handlers behind the current one in a chain of event handlers
  /// will not get called.
 ...
 *
 *   It connect the functions to be called as response to the event and manages
 * that chain of responses It is a functor, that get called to connect a
 * "normal" response function, with normal "priority". If a response function
 * need another priority (unignorable or called first) it will need to be
 * connected with the specific connect function not with the operator() It also
 * permit to "emit" that event, calling all the active responders.
 */
template <typename Arg> class basic_event : public detail::event_base {
public:
  using arg_reference = const typename std::remove_reference<Arg>::type &;

private:
  struct docker : public detail::docker_base {
    /// the callback/response function taking the typed argument
    std::function<void(arg_reference)> invoke;

    docker(basic_event *evt, std::function<void(arg_reference)> &&ivk,
           bool unignorable_flag)
        : docker_base(evt, unignorable_flag), invoke(std::move(ivk)) {}

    docker(basic_event *evt, const std::function<void(arg_reference)> &ivk,
           bool unignorable_flag)
...
public:
  ...
  template <typename Function> event_handle connect_front(Function &&fn) {
    ...
    if constexpr (std::is_invocable_v<Function, arg_reference>) {
      ...
      return _m_emplace(new docker{this, [fn](arg_reference) { fn(); }, false},
                        true);
    }
#else
    ...
    return _m_emplace(
        new docker(
            this,
            factory<prototype, std::is_bind_expression<prototype>::value>::
                build(std::forward<Function>(fn)),
            false),
        true);
#endif
  }
  ...
  event_handle connect(void (*fn)(arg_reference)) {
    return connect([fn](arg_reference arg) { fn(arg); });
  }
#endif

  /// It will not get called if stop_propagation() was called, because it is set
  /// at the end of the chain..
  template <typename Function> event_handle connect(Function &&fn) {
#ifdef __cpp_if_constexpr
    if constexpr (std::is_invocable_v<Function, arg_reference>) {
      return _m_emplace(new docker{this, fn, false}, false);
    } else if constexpr (std::is_invocable_v<Function>) {
      return _m_emplace(
          new docker{this, [fn](arg_reference) mutable { fn(); }, false},
          false);
    }
#else
    using prototype = typename std::remove_reference<Function>::type;
  ...
  /// It will not get called if stop_propagation() was called.
  template <typename Function> event_handle operator()(Function &&fn) {
  ...
  event_handle connect_unignorable(Function &&fn, bool in_front = false) {
    ...
    if constexpr (std::is_invocable_v<Function, arg_reference>) {
      ...
      return _m_emplace(
          new docker{this, [fn](arg_reference) mutable { fn(); }, true},
          in_front);
    }
#else
    using prototype = typename std::remove_reference<Function>::type;
  ...
  void emit(arg_reference &arg, window window_handle) {
    ...
    // The dockers may resize when a new event handler is created by a calling
    // handler. Traverses with position can avaid crash error which caused by a
    // iterator which becomes invalid.
...
private:
#ifndef __cpp_if_constexpr
  template <typename Fn, bool IsBind> struct factory {
    static std::function<void(arg_reference)> build(Fn &&fn) {
      return std::move(fn);
    }

    static std::function<void(arg_reference)> build(const Fn &fn) { return fn; }
  };

  template <typename Fn> struct factory<Fn, false> {
    ...
    static std::function<void(arg_reference)> build_second(Tfn &&fn,
                                                           Ret (fn_type::*)()) {
      return [fn](arg_reference) mutable { fn(); };
    }

    template <typename Tfn, typename Ret>
    static std::function<void(arg_reference)>
    build_second(Tfn &&fn, Ret (fn_type::*)() const) {
      return [fn](arg_reference) mutable { fn(); };
    }

    template <typename Tfn, typename Ret>
    static std::function<void(arg_reference)>
    build_second(Tfn &&fn, Ret (fn_type::*)(arg_reference)) {
      return std::forward<Tfn>(fn);
    }

    template <typename Tfn, typename Ret>
    static std::function<void(arg_reference)>
    build_second(Tfn &&fn, Ret (fn_type::*)(arg_reference) const) {
      return std::forward<Tfn>(fn);
    }

    template <typename Tfn, typename Ret, typename Arg2>
    static std::function<void(arg_reference)>
    build_second(Tfn &&fn, Ret (fn_type::*)(Arg2)) {
      static_assert(
          std::is_convertible<arg_type, Arg2>::value,
          "The parameter type is not allowed, please check the function "
          "parameter type where you connected the event function.");
      return [fn](arg_reference arg) mutable { fn(arg); };
    }

    template <typename Tfn, typename Ret, typename Arg2>
    static std::function<void(arg_reference)>
    build_second(Tfn &&fn, Ret (fn_type::*)(Arg2) const) {
      static_assert(
          std::is_convertible<arg_type, Arg2>::value,
          "The parameter type is not allowed, please check the function "
          "parameter type where you connected the event function.");
  ...
  struct factory<std::function<Ret(Arg2)>, false> {
    ...
    static std::function<void(arg_reference)>
    build(const std::function<Ret(Arg2)> &fn) {
      return [fn](arg_reference arg) mutable { fn(arg); };
    }

    static std::function<void(arg_reference)>
    build_second(std::function<void(arg_reference)> &&fn) {
      return std::move(fn);
    }
  };

  template <typename Ret> struct factory<std::function<Ret()>, false> {
    static std::function<void(arg_reference)>
    build(const std::function<Ret()> &fn) {
      return [fn](arg_reference) mutable { fn(); };
    }
  };

  template <typename Ret> struct factory<Ret (*)(), false> {
    static std::function<void(arg_reference)> build(Ret (*fn)()) {
      return [fn](arg_reference) mutable { fn(); };
    }
  };

  template <typename Ret, typename Arg2> struct factory<Ret (*)(Arg2), false> {
    typedef typename std::remove_reference<arg_reference>::type arg_type;
    static_assert(
        std::is_convertible<arg_type, Arg2>::value,
        "The parameter type is not allowed, please check the function "
  ...
  template <typename Ret, typename Arg2> struct factory<Ret(Arg2), false> {
    ...
    static std::function<void(arg_reference)> build(Ret (*fn)(Arg2)) {
      return [fn](arg_reference arg) { fn(arg); };
    }
  };
#endif
};
...
struct arg_mouse : public event_arg {
  ...
  bool is_left_button() const {
    return (event_code::mouse_move == evt_code
                ? left_button
                : (mouse::left_button == button));
  }
};
...
struct arg_wheel : public arg_mouse {
  ...
  bool upwards; ///< true if the wheel is rotated to the top/left, depends on
                ///< which and false otherwise
...
struct arg_focus : public event_arg {
  ...
  ::nana::window window_handle; ///< A handle to the event window
  ::nana::native_window_type
      receiver; ///< it is a native window handle, and specified which window
                ///< receives focus
  bool getting; ///< the window received focus?
  reason focus_reason; ///< determines how the widget receives keyboard focus,
                       ///< it is ignored when 'getting' is equal to false
};

struct arg_ime : public event_arg {
  enum class reason { composition, result };
...
struct arg_keyboard : public event_arg {
  ...
  mutable bool ignore; ///< this member is only available for key_char event,
                       ///< set 'true' to ignore the input.
  bool alt;   ///< it is set to indicate the modifier key Alt just prior to the
              ///< event.
  bool ctrl;  ///< it is set to indicate the modifier key Ctrl just prior to the
              ///< event.
  bool shift; ///< it is set to indicate the modifier key Shift just prior to
              ///< the event.
};
...
struct arg_resizing : public event_arg {
  ...
  mutable unsigned width;  ///< new width in pixels. If it is modified, the
                           ///< window's width will be the modified value
  mutable unsigned height; ///< new height in pixels. If it is modified, the
                           ///< window's height will be the modified value
};
...
struct arg_click : public event_arg {
  ::nana::window window_handle; ///< A handle to the event window
  const arg_mouse
      *mouse_args{}; ///< If it is not null, it refers to the mouse arguments
                     ///< for click event emitted by mouse, nullptr otherwise.
};
...
struct general_events {
  ...
  basic_event<arg_click> click;     ///< the window is clicked, but occurs after
                                    ///< mouse_down and before mouse_up
  basic_event<arg_mouse> dbl_click; ///< the window is double clicked
  basic_event<arg_wheel>
      mouse_wheel; ///< the mouse wheel rotates while the window has focus
  basic_event<arg_dropfiles>
      mouse_dropfiles; ///< the mouse drops some external data while the window
                       ///< enable accepting files
  ...
  basic_event<arg_keyboard>
      key_press; ///< a key is pressed while the window has focus. event code is
                 ///< event_code::key_press
  basic_event<arg_keyboard>
      key_release; ///< a key is released while the window has focus. event code
                   ///< is event_code::key_release
  basic_event<arg_keyboard>
      key_char; ///< a character, whitespace or backspace is pressed. event code
                ///< is event_code::key_char
  basic_event<arg_keyboard> shortkey; ///< a defined short key is pressed. event
                                      ///< code is event_code::shortkey
  ...
  basic_event<arg_destroy> destroy; ///< the window is destroyed, but occurs
                                    ///< when all children have been destroyed
};

namespace detail {
struct events_root_extension : public general_events {
