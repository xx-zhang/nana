...
public:
   ...
   * @param limit_pixels The number of pixels of the limited edge. If this
   * parameter is zero, it is ignored.
   * @param limit_width True if limits the width, false if limits the height.
   * @return the size of content.
   */
  virtual ::std::optional<size> measure(graph_reference graph,
                                        unsigned limit_pixels,
                                        bool limit_width) const = 0;
