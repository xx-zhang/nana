...
public:
  ...
  static void
  set_missing(std::function<void(const std::string &msgid_utf8)> handler);
...
private:
  ...
  void _m_replace_args(::std::string &str,
                       std::vector<::std::string> *arg_strs) const;

#ifndef __cpp_fold_expressions
  static void
  _m_fetch_args(std::vector<std::string> &); // Termination of _m_fetch_args
#endif

  static void _m_fetch_args(std::vector<std::string> &v, const char *arg);
  static void _m_fetch_args(std::vector<std::string> &v,
...
#ifndef __cpp_fold_expressions
  template <typename... Args>
  void _m_fetch_args(std::vector<std::string> &v, const char *arg,
                     Args &&...args) const {
    v.emplace_back(arg);
  ...
  template <typename... Args>
  void _m_fetch_args(std::vector<std::string> &v, const std::string &arg,
                     Args &&...args) const {
    v.emplace_back(arg);
    _m_fetch_args(v, std::forward<Args>(args)...);
  }
  ...
  template <typename... Args>
  void _m_fetch_args(std::vector<std::string> &v, std::string &arg,
                     Args &&...args) const {
    v.emplace_back(arg);
    _m_fetch_args(v, std::forward<Args>(args)...);
  }
  ...
  template <typename... Args>
  void _m_fetch_args(std::vector<std::string> &v, const wchar_t *arg,
                     Args &&...args) const {
  ...
  template <typename... Args>
  void _m_fetch_args(std::vector<std::string> &v, const std::wstring &arg,
                     Args &&...args) const {
    v.emplace_back(to_utf8(arg));
    _m_fetch_args(v, std::forward<Args>(args)...);
  }
  ...
  template <typename... Args>
  void _m_fetch_args(std::vector<std::string> &v, std::wstring &arg,
                     Args &&...args) const {
    v.emplace_back(to_utf8(arg));
    _m_fetch_args(v, std::forward<Args>(args)...);
  }
  ...
  void _m_fetch_args(std::vector<std::string> &v, Arg &&arg,
                     Args &&...args) const {
    std::stringstream ss;
    ss << arg;
    v.emplace_back(ss.str());
...
class i18n_eval {
  ...
  template <typename Return> class arg_function : public eval_arg {
  public:
    arg_function(std::function<Return()> fn) : fn_(fn) {}
...
public:
  ...
  // Workaround for VC2013, becuase it can't specified a default explicit
  // move-constructor
...
private:
  ...
  template <typename Arg> void _m_add_args(Arg &&arg) {
    std::stringstream ss;
    ss << arg;
    _m_add_args(ss.str());
  }

  template <typename Return> void _m_add_args(std::function<Return()> fn) {
