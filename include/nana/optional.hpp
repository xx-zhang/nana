 ...
 *
 *	@brief An implementation of experimental library optional of C++
 *standard(http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2013/n3793.html)
 */
...
public:
  ...
  template <typename U> storage(U &&value) : initialized_{true} {
    ::new (data_) value_type(std::forward<U>(value));
  }

  template <typename U> storage(const U &value) : initialized_{true} {
    ::new (data_) value_type(value);
  }

  storage(const storage &other) : initialized_{other.initialized_} {
    if (other.initialized_)
      ::new (data_) value_type(*other.ptr());
  }

  storage(storage &&other) : initialized_{other.initialized_} {
    if (other.initialized_)
      ::new (data_) value_type(std::move(*other.ptr()));
  }

  template <typename U>
  storage(const storage<U> &other) : initialized_{other.initialized_} {
    if (other.initialized_)
      ::new (data_) value_type(*other.ptr());
  }
...
public:
  ...
  optional(const optional &other) : storage_(other.storage_) {}

  optional(optional &&other) : storage_(std::move(other.storage_)) {}

  constexpr optional(const value_type &value) : storage_(value) {}

  constexpr optional(value_type &&value) : storage_(std::move(value)) {}
  ...
  // Value access
  // constexpr
  value_type *operator->() { return storage_.ptr(); }
  constexpr const value_type *operator->() const { return storage_.ptr(); }
  ...
  */
  ...
  // Condition
  constexpr explicit operator bool() const { return storage_.initialized(); }
