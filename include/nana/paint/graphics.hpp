...
public:
  ...
  /**
   * @param fixed Indicates whether to return a fixed font size. If this
   * parameter is false, the method may return zero for default system font
   * size. If the parameter is true, the method returns a fixed size of default
   * font size if the font size that assigned by constructor is zero.
...
public:
  ...
  graphics(const graphics &); ///< the resource is not copyed, the two graphics
                              ///< objects refer to the *SAME* resource
  ...
  bool empty() const; ///< Returns true if the graphics object does not refer to
                      ///< any resource.
  ...
  /**
   * @param sz The dimension of the graphics to be requested. If sz is empty, it
   * performs as release().
   */
  void make(const ::nana::size &sz);
  void resize(const ::nana::size &);
  void typeface(const font &); ///< Selects a specified font type into the
                               ///< graphics object.
  font typeface() const;
  ...
  /// Only supports the wide string, because it is very hard to specify the
  /// begin and end position in a UTF-8 string.
  ::nana::size glyph_extent_size(std::wstring_view text, std::size_t begin,
                                 std::size_t end) const;

  /// Returns a buffer which stores the pixel of each charater stored in text.
  /**
   * @param text The text to be requested.
   * @return A buffer which stores the pixel of each character stored in text,
   * its length is same with text's length. If text is empty, it returns a
   * buffer with a senseless value.
   */
  ...
  ::nana::size text_extent_size(const wchar_t *)
      const; ///< Computes the width and height of the specified string of text.
  ::nana::size text_extent_size(const ::std::wstring &)
      const; ///< Computes the width and height of the specified string of text.
  ::nana::size text_extent_size(const wchar_t *, std::size_t length)
      const; ///< Computes the width and height of the specified string of text
             ///< with the specified length.
  ::nana::size text_extent_size(const ::std::wstring &, std::size_t length)
      const; ///< Computes the width and height of the specified string of text
             ///< with the specified length.

  ::nana::size glyph_extent_size(const wchar_t *, std::size_t length,
                                 std::size_t begin, std::size_t end) const;
  ::nana::size glyph_extent_size(const ::std::wstring &, std::size_t length,
                                 std::size_t begin, std::size_t end) const;
  ...
  bool text_metrics(unsigned &ascent, unsigned &descent,
                    unsigned &internal_leading) const;

  void line_begin(int x, int y);

  void bitblt(int x, int y,
              const graphics
                  &source); ///<   Transfers the source to the specified point.
  void bitblt(
      const ::nana::rectangle &r_dst,
      native_window_type src); ///< Transfers the color data corresponding to
                               ///< r_dst from the src window to this graphics.
  void bitblt(const ::nana::rectangle &r_dst, native_window_type src,
              const point &p_src); ///< Transfers the color data corresponding
                                   ///< to r_dst from the src window at point
                                   ///< p_src to this graphics.
  void bitblt(
      const ::nana::rectangle &r_dst,
      const graphics &src); ///< Transfers the color data corresponding to r_dst
                            ///< from the src graphics to this graphics.
  void bitblt(const ::nana::rectangle &r_dst, const graphics &src,
              const point &p_src); ///< Transfers the color data corresponding
                                   ///< to r_dst from the src graphics at point
                                   ///< p_src to this graphics.

  void blend(const ::nana::rectangle &r, const ::nana::color &,
             double fade_rate);
  void blend(const ::nana::rectangle &blend_r, const graphics &blend_graph,
             const point &blend_graph_point,
             double fade_rate); ///< blends with the blend_graph.

  void blur(const ::nana::rectangle &r, std::size_t radius); ///< Blur process.

  void
  paste(graphics &dst, int x,
        int y) const; ///< Paste the graphics object into the dest at (x, y)
  void paste(native_window_type dst, const ::nana::rectangle &, int sx,
             int sy) const; ///< Paste the graphics object into a
                            ///< platform-dependent window at (x, y)
  void paste(native_window_type dst, int dx, int dy, unsigned width,
             unsigned height, int sx, int sy) const;
  ...
  void stretch(const ::nana::rectangle &src_r, graphics &dst,
               const ::nana::rectangle &r) const;
  ...
  void frame_rectangle(const ::nana::rectangle &, const color &left,
                       const color &top, const color &right,
                       const color &bottom);
  void frame_rectangle(const ::nana::rectangle &, const color &, unsigned gap);

  void gradual_rectangle(const ::nana::rectangle &, const color &from,
                         const color &to, bool vertical);
  void round_rectangle(const ::nana::rectangle &, unsigned radius_x,
                       unsigned radius_y, const color &, bool solid,
