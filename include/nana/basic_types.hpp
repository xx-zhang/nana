...
struct casei_char_traits : public std::char_traits<CharT> {
...
/// See extended CSS color keywords (4.3) in
...
public:
  ...
  color &from_rgb(unsigned red, unsigned green,
  ...
  color &from_hsl(double hue, double saturation,
                  double lightness); ///< immutable alpha channel

  /// Blends color
  /**
   * Returns a color which is blended as this * (1 - fade_rate) + blending_color
   * * fade_rate
...
template <typename T> struct basic_point {
  // typedef-names
  using value_type = T;
...
struct size {
  ...
  bool is_hit(const point &)
      const; ///< Assume it is a rectangle at (0,0), and check whether a
             ///< specified position is in the rectangle.
  size &shift();
...
struct rectangle {
  ...
  explicit rectangle(
      const size &); ///< a rectangle with specified size at coordinate (0, 0).
  ...
  /**
   * It's equal to x += pixels; y + pixels; width -= (pixels << 1); height -=
   * (pixels << 1);
   * @param pixels The number of pixels to be pared. If the number that
   * multiples pixels twice is larger than width/height, the width/height will
   * be zero. If the pixels is a negative number, the width/height is add the
   * number that multiple pixels twice.
...
enum class arrange { unknown, horizontal, vertical, horizontal_vertical };
...
enum class direction { north, south, east, west, southeast };
} // end namespace nana
